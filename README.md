...
- Browsers: API mode (no browser), Chrome Desktop and Chrome Mobile Emulator
- CI/CD: Github Actions using a Self-Hosted Runner
- Database: PostgreSQL
- AI Tools: Playwright MCP, custom Skills, Dependabot (Github Actions)

## Highlights:
- Test Website: Tests are executed against a WordPress test website hosted on a 3rd party Web Hosting platform. The site has typical WordPress features such as Login, Lost Password, User Types, User Profile settings and creating/editing blog posts
...
- Security: Files with sensitive data (ex. credentials, configuration details) are not saved in the repository. The data is stored in Github Secrets encrypted environment variables. The data is provided temporarily during Github cloud test run executions but destroyed upon test run completion (see Self-Hosted Runner notes below). It never appears in logs or test result reports.
...
* create-jira-bug-ticket-exploratory-testing: creates a Jira bug ticket for each test failure from an AI agent's Playwright MCP exploratory testing session
...
* The MCP server provides an AI agent with various Playwright tools to perform UI or Accesibility testing in a browser and provide feedback such as Pass/Fail results or take screenshots
* The MCP configuration is configured to launch the browser in headed mode by default. This is preferred because headless mode browsers can sometimes return test failures that are false positives.
